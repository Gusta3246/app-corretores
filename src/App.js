            ...
            {pdfLeitor && (() => {
                ...
                return (
                    <div className="fixed inset-0 z-[60] flex flex-col" style={{background:'#0f0f0f'}}>
                        ...
                        {/* Barra de botões — abaixo da status bar */}
                        <div
                            className="flex-shrink-0 flex items-center justify-between px-4 pb-3"
                            style={{
                                paddingTop: 'max(env(safe-area-inset-top, 0px) + 12px, 48px)',
                                background: 'linear-gradient(to bottom, rgba(0,0,0,0.80) 0%, rgba(0,0,0,0.4) 80%, transparent 100%)',
                                position: 'relative',
                                zIndex: 20,
                            }}
                        >
                            ...
                            <button
                                ...
                                className="w-11 h-11 flex items-center justify-center rounded-full text-white active:scale-95 transition-all"
                                style={{background:'rgba(0,0,0,0.65)', backdropFilter:'blur(8px)', WebkitBackdropFilter:'blur(8px)', border:'1px solid rgba(255,255,255,0.18)'}}
                            >
                                <X size={20} />
                            </button>
                            {/* Título */}
                            <span className="text-white/80 text-xs font-semibold truncate max-w-[150px] text-center">{pdfLeitor.title}</span>
                            {/* Drive + WhatsApp */}
                        ...
                        {/* Área central — sem iframe para não travar o app */}
                        <div className="flex-1 flex flex-col items-center justify-center gap-6 px-6" style={{background:'#0f0f0f'}}>
                            <div className={`w-20 h-20 rounded-3xl flex items-center justify-center ${isDir ? 'bg-orange-500/20' : 'bg-blue-500/20'}`}
                                style={{border: isDir ? '1.5px solid rgba(249,115,22,0.35)' : '1.5px solid rgba(59,130,246,0.35)'}}>
                                <BookOpen size={38} className={isDir ? 'text-orange-400' : 'text-blue-400'} />
                            </div>
                            <div className="text-center">
                                <p className="text-white font-black text-lg">{pdfLeitor.title}</p>
                                <p className="text-white/50 text-xs mt-1">{pdfLeitor.brand} · Revista Digital</p>
                            </div>
                            <a
                                href={driveViewUrl}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="w-full max-w-xs py-4 rounded-2xl text-white text-sm font-black uppercase tracking-widest active:scale-[0.97] transition-all flex items-center justify-center gap-2"
                                style={{background: isDir ? 'linear-gradient(135deg,#f97316,#ef4444)' : 'linear-gradient(135deg,#6366f1,#7c3aed)', boxShadow: isDir ? '0 4px 20px rgba(249,115,22,0.4)' : '0 4px 20px rgba(99,102,241,0.4)'}}
                            >
                                <ExternalLink size={16} /> Abrir Revista
                            </a>
                            <a
                                href={whatsappUrl}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="w-full max-w-xs py-4 rounded-2xl text-sm font-black uppercase tracking-widest active:scale-[0.97] transition-all flex items-center justify-center gap-2"
